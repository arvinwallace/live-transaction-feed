...
export default function Home() {
  ...
  // initialize Web3 with the awesome key you guys gave me.. Thank you!
  var web3 = new Web3(process.env.WEB3_KEY);

  let subscription = useRef()

  useEffect(() => {
    // Listen for incoming header for latest block
    subscription.current = web3.eth.subscribe('newBlockHeaders')
      .on("data", function (blockHeader) {

        // The block header gives us a hash number 
        // we can now get block data with it
        web3.eth.getBlock(blockHeader.hash).then(block => {

          // initialize an array to collect all the transactions
          ...
          // Don't forget to catch potential errors from initial block retrieval call
          .catch(err => console.error(err))
      })
      .on("error", () => {console.error("Something went wrong with the Subscription")});
  ...
  // Display loading screen until we get our first non-empty block
